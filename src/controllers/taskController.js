...
export const addTask = async (req, res) => {
  const { projectId, title, description, status, assigneeId, deadline } =
    req.body;
  ...
  try {
    ...
    if (!req.user.isAdmin) {
      ...
      if (!member || !['OWNER', 'ADMIN'].includes(member.role)) {
        return res
          .status(403)
          .json({ message: 'Only project owners or admins can add tasks' });
      }
    }

    // Check If Assignee Person Exists in DB !?
    const assigneExists = await ProjectMember.findOne({
      userId: assigneeId,
      projectId,
    });
    ...
    res
      .status(500)
      .json({ message: 'Internal server error while adding task' });
  }
};
  ...
  try {
    ...
    if (!req.user.isAdmin) {
      ...
      if (!member || !['OWNER', 'ADMIN'].includes(member.role)) {
        return res
          .status(403)
          .json({ message: 'Only owners or admins can update tasks' });
      }
    }
...
export const updateTaskStatus = async (req, res) => {
  ...
  try {
    ...
    // Only Assignee can update the Task Status. (Admin allowed too)
    if (!req.user.isAdmin && task.assigneeId?.toString() !== userId) {
      return res
        .status(403)
        .json({ message: 'Only the assignee can update the task status' });
    }
...
export const getAssignedTasks = async (req, res) => {
  ...
  try {
    const tasks = await Task.find({ assigneeId: userId }).populate(
      'assigneeId',
      'username email',
    );
    ...
    res.status(500).json({
      message: 'Internal server error while getting assigned tasks',
    });
  }
};
...
export const deleteTask = async (req, res) => {
  ...
  try {
    ...
    // Only Owner , Admin Can Delete (and System Admin)
    if (!req.user.isAdmin) {
      const member = await ProjectMember.findOne({ userId, projectId });
      if (!member || !['OWNER', 'ADMIN'].includes(member.role)) {
        return res
          .status(403)
          .json({ message: 'Only owners or admins can delete tasks' });
      }
    }
