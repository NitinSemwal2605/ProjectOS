...
export const getMessages = async (req, res) => {
    try {
        const { projectId, limit, offset } = req.query; // send in query for pagination
        ...
        const data = await messageService.getProjectMessages(projectId, limit, offset);
...
export const editMessage = async (req, res) => {
    try {
        ...
        if (!message) {
            return res.status(404).json({ message: "Message not found or unauthorized to edit" });
        }
