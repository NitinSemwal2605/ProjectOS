...
export const registerChatHandlers = (io, socket) => {
    ...
    socket.on("chat:send", async (data) => {
        try {
            ...
            if (!socket.rooms.has(room)) {
                console.warn(`User ${socket.user.id} attempted to send message to project ${projectId} without being a member.`);
                return;
            }

            // Save message to DB
            const message = await messageService.saveMessage(projectId, socket.user.id, content);
            // console.log(message)
    ...
    socket.on("chat:edit", async (data) => {
        try {
            ...
            const message = await messageService.editMessage(messageId, socket.user.id, content);
    ...
    socket.on("chat:delete", async (data) => {
        try {
            ...
            // Soft delete
            const message = await messageService.deleteMessage(messageId, socket.user.id);
